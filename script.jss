...
function loadSavedData() {
    ...
    // 3. Load Co-located players.csv automatically
    fetch('players.csv')
        .then(response => {
            if (!response.ok) {
...
function resetPlayersPool() {
    if (confirm("Are you completey clearing all player data and draft state?")) {
...
function startTimer() {
    ...
    isTimerRunning = document.getElementById("timerToggleBtn") ? true : true;
